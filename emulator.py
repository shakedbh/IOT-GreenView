# Class of GreenView IOT devices (emulators) like: DHT, Elec meter, water meter etc.
import sys
import random
...
class ConnectionDock(QDockWidget):
    ...
    def update_btn_state(self, msg):
        global tmp_upd
        if 'Set' in msg:
            tmp = '12'
            self.ePushtbtn.setStyleSheet("background-color: green")
            try:
                tmp = msg.split('Set temperature to: ')[1]
...
class MainWindow(QMainWindow):
    def __init__(self, args, parent=None):
        ...
        if 'DHT' in self.name:
            ...
            self.timer.start(int(self.update_rate) * 1000)  # in msec
        elif 'Temperature' in self.name:
            # Creating timer for update rate support
            self.timer = QtCore.QTimer(self)
            self.timer.timeout.connect(self.create_data_temp)
            self.timer.start(int(self.update_rate) * 1000)  # in msec
        elif 'light' in self.name:
            tmp_upd = -5
            # Creating timer for update rate support
            self.timer = QtCore.QTimer(self)
            self.timer.timeout.connect(self.create_data_temp)
    ...
    def create_data_light(self):
        ic('light data update')
        if not self.mc.connected:
            self.connectionDock.on_button_connect_click()
        if not self.mc.subscribed:
            self.mc.subscribe_to(self.topic_sub)

    def create_data_temp(self):
        global tmp_upd
        ic('Temperature data update')
        if not self.mc.connected:
            self.connectionDock.on_button_connect_click()
        if not self.mc.subscribed:
            self.mc.subscribe_to(self.topic_sub)
